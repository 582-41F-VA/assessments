...
- The page has two sections: controls on the left and scoreboard on the
  right.
- The controls section has an `<input>` for players to enter words,
  either by typing on their keyboard when the `<input>` is focused, or
  clicking on the letters in the honeycomb.
...
[Classroom]: https://classroom.github.com/a/9XsJD3Aw
